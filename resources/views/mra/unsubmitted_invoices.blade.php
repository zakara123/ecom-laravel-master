<x-app-layout>
                ...
                <li class="inline-flex items-center">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-400 ml-1 md:ml-2 text-sm font-medium" aria-current="page">MRA</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">All Unsubmitted Invoices</h1>
        </div>
    </x-slot>
    ...
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
        ...
        <div class="mb-1 w-full">
            <form class="sm:pr-3 mb-4 sm:mb-0" id="form_search" action="javascript:void(0)" data-action="{{ route('search-sales') }}" method="GET">
            <div class="block sm:flex items-center md:divide-x md:divide-gray-100">
                ...
                <div class="flex items-center sm:justify-end w-full">
                    <div class="hidden md:flex pl-2 space-x-1">
                        ...
                        <a href="{{ route('mra.send_unsumbitted_invoices') }}"
                ...
                <div class="shadow overflow-hidden">
                    <table class="table-fixed min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($paginatedResults as $mra)
                            <tr class="hover:bg-gray-100">
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">@if($mra->sale_id){{ $mra->sale_id }}@else{{ $mra->sales_id }}@endif</td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900"> {{ date('d/m/Y H:i', strtotime($mra->created_at)) }}</td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">{{ $mra->requestId }}</td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">{{ $mra->status }}</td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">{{ $mra->invoiceIdentifier }}</td>
                                <td class="p-4 whitespace-nowrap space-x-2">
                                    @if($mra->sale_id)
                                        <a href="{{ route('detail-sale', ['id' => $mra->sale_id]) }}" data-modal-toggle="product-modal" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                    ...
                                    @else
                                        <a href="{{ route('detail-sale', ['id' => $mra->sales_id]) }}" data-modal-toggle="product-modal" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye mr-2" viewBox="0 0 16 16">
                                                <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z" />
                                                <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z" />
                                            </svg>
                    ...
                    {{ $paginatedResults->links("pagination::tailwind") }}
                    </div>
                </div>
            </div>
        </div>
