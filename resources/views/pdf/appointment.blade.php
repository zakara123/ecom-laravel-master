...
<body>
        ...
        <table style="width:100%">
            <tr>
                <td>
                    <br><b>Appointment To:</b>
                    <br>{{$appointment->patient_firstname}} {{$appointment->patient_lastname}}
                    @if(!empty($appointment->patient_address))<br>{{$appointment->patient_address}}@endif
                    @if(!empty($appointment->patient_city))<br>{{$appointment->patient_city}}@endif
                    @if(!empty($appointment->patient_email))<br>{{$appointment->patient_email}}@endif
                    @if(!empty($appointment->patient_phone))<br>{{$appointment->patient_phone}}@endif
                    @if(isset($appointment->order_reference) && !empty($appointment->order_reference)) <br><br>appointment Ref: {{$appointment->order_reference}} @endif

                </td>
                <td style="width:100px"></td>
                <td style="text-align:right">
                    <br><br>Invoice Number: {{$appointment->invoice_number}}
                    <br>Appointment Date: {{ date('d/m/Y', strtotime($appointment->created_at)) }}
                </td>
            </tr>
        </table>
        <h3>
