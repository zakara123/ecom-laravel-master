                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="grid md:grid-cols-2 md:gap-6">
                                <div class="mb-6">
                                    ...
                                    <input
                                        ...
                                        type="text" name="order_reference" placeholder="Order Reference"
                                    ...
                                    <select id="store" name="store"
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{ $store->id }}"
                                                    @if($store->id == old('store')) selected @endif>{{ $store->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <select id="customer" name="customer"
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}"
                                                    @if($customer->id == old('customer')) selected @endif>{{ $customer->firstname }} {{ $customer->lastname }}
                                                ({{$customer->email}})
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid md:grid-cols-3 md:gap-6">
                                    ...
                                    <select id="item" name="item"
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}"
                                                    @if($product->id == old('item')) selected @endif>{{ $product->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-6">
                                ...
                                <div class="mb-6">
                                    <button name="add_item" id="add_item"
                                            class="bg-blue-700 mt-7 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">
                                        ADD
                                    </button>
                                </div>
                            </div>
                            <div class="overflow-x-auto relative">
                                <table class="w-full text-xs text-center font-medium text-gray-900 dark:text-gray-400">
                                    <thead
                                        class="text-xs border text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                            ...
                            <div class="w-full mt-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input id="delivery_date" name="delivery_date" datepicker datepicker-autohide
                                           minDate="{{date('d/m/Y', strtotime(time()))}}" datepicker-format="dd/mm/yyyy"
                                           value="{{ old('delivery_date') }}" type="text"
                                ...
                                <select id="payment_method" name="payment_method"
                                    ...
                                    @foreach($payment_mode as $payment)
                                        <option value="{{ $payment->id }}"
                                                @if($payment->id == old('payment_method')) selected @endif>{{ $payment->payment_method }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-full mt-4">
                                <button type="submit" name="add_sale" id="add_sale"
