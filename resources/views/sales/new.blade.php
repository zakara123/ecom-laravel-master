<x-app-layout>
    ...
    <form action="{{ route('newsale.index') }}" id="main_form" method="post">
        @csrf
        <input type="hidden" name="type_sale" id="type_sale" value="BACK_OFFICE_SALE">
        ...
        <div class="py-2">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="mb-6 grid md:grid-cols-3 md:gap-6">
                            <div class="flex mb-2">
                                <h1 class="text-2xl sm:text-xl font-semibold text-gray-900">{{ __('New Sale') }}</h1>
                                <input name="order_reference"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/2 ml-4 p-2.5"
                                    type="text" placeholder="Order Reference" value="{{ old('order_reference') }}">
                            </div>
                            <div class="col-span-2 text-right mx-4">
                                <button data-modal-toggle="add-new-customer" id="btn-add-new-customer" type="button"
                                    class="bg-blue-700 hover:bg-blue-700 mx-4 text-white font-bold py-2 px-4 rounded cursor-pointer">New
                                    Customer</button>
                                <button name="name-add-news-item" id="id-add-news-item" type="button"
                                    class="bg-blue-700 hover:bg-blue-700 mr-4 text-white font-bold py-2 px-4 rounded cursor-pointer">Add
                                    New Item</button>
                                <button data-modal-toggle="add-service-item" name="add_service_item"
                                    id="add_service_item" type="button"
                                    class="bg-blue-700 hover:bg-blue-700 mr-4 text-white font-bold py-2 px-4 rounded cursor-pointer">Add
                                    Service Item</button>
                            </div>
                        </div>
                        <div class="mx-auto my-2">
                                        ...
                                        <select id="customer" name="customer"
                                            ...
                                            @foreach ($customers as $customer)
                                                <option value="{{ $customer->id }}"
                                                    @if (Session::has('old_customer') && Session::get('old_customer') !== null) @if (Session::get('old_customer') == $customer->id)
                                                                selected @endif
                                                @else
                                                    @if (empty(old('customer'))) @if ($customer->is_default == 'yes')
                                                                    selected @endif
                                                @else @if ($customer->id == old('customer')) selected @endif
                                                    @endif
                                            @endif
                                            >
                                            ...
                                            @else
                                                {{ $customer->firstname }} {{ $customer->lastname }} @endif @if (!empty($customer->email))
                                                    ({{ $customer->email }})
                                                @endif
                                                </option>
                                            @endforeach
                                        </select>
                                        <small id="error_message_customer" class="text-red-600 ml-2 hidden">Customer
                                    ...
                                    <select id="store" name="store"
                                        ...
                                        @foreach ($stores as $store)
                                            <option value="{{ $store->id }}"
                                                @if ($store->id == old('store')) selected @endif
                                                @if ($store->id == old('store_popup')) selected @endif
                                                @if (!empty($store_first) && $store->id == $store_first) selected @endif>{{ $store->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <small id="error_message_store" class="text-red-600 ml-2 hidden">Store field is
                                        required</small>
                                </div>
                            </div>
                            <div class="grid md:grid-cols-3 md:gap-6">
                                        ...
                                        <select id="item" name="item"
                                            ...
                                            @foreach ($items as $product)
                                                <option value="{{ $product->id }}"
                                                    @if (Session::has('old_item')) @if (Session::get('old_item') == $product->id) selected @endif
                                                    @endif>{{ $product->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <small id="stock_label" class="text-gray-900 ml-2 hidden">Current Stock : 0</small>
                                    <small id="error_message_item" class="text-red-600 ml-2 hidden">Item field is
                                        required</small>
                                </div>
                                ...
                                <div class="mb-6">
                                    ...
                                    <input type="number" name="quantity" id="quantity"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                        placeholder="">
                                    <small id="error_message_quantity" class="text-red-600 ml-2 hidden">Quantity field
                                        is required</small>
                                </div>
                                <div class="mb-6">
                                    <input type="submit" name="add_item" id="add_item" value="Add"
                                        class="bg-blue-700 mt-7 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded cursor-pointer"></input>
                                    <input type="button" id="label_click_modal_add_item_global"
                                        value="modal add item" style="display:none"
                            ...
                            <div class="overflow-x-auto relative">
                                <table class="w-full text-xs text-center font-medium text-gray-900">
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            $isGlobalDiscount = true; // To check if all discounts are the same
                                            $firstDiscount = isset($newsale[0]) ? $newsale[0]->discount : 0; // Get the discount of the first item
                                        @endphp
                                        @foreach ($newsale as $item)
                                            @php
                                                // Add up the discount
                                                $totalDiscount = $firstDiscount;

                                                // Check if this item's discount is different from the first item's discount
                                                if ($item->discount != $firstDiscount) {
                                            ...
                                            <tr>
                                                ...
                                                <td class="py-3 px-6 border">
                                                    @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                        {{ number_format($item->product_price_converted, 2, '.', ',') }}
                                                ...
                                                @if ($vat_type != 'No VAT')
                                                    <td class="py-3 px-6 border">
                                                        @if ($item->tax_sale == '15% VAT' && $vat_type != 'No VAT')
                                                            @if (!empty(old('currency')) && old('currency') == 'MUR')
                                                                {{ number_format($item->product_price * 0.15 * $item->quantity, 2, '.', ',') }}
                                                                <br><small>(15% VAT)</small>
                                                            @else
                                                                {{ number_format($item->product_price_converted * 0.15 * $item->quantity, 2, '.', ',') }}
                                                                ({{ number_format($item->product_price * 0.15 * $item->quantity, 2, '.', ',') }})
                                                                <br><small>(15% VAT)</small>
                                                            @endif
                                                            @php
                                                                $vat_amount =
                                                                    $vat_amount +
                                                                    $item->product_price * 0.15 * $item->quantity;
                                                                $tax_amount_converted =
                                                                    $tax_amount_converted +
                                                                    $item->product_price_converted *
                                                                        0.15 *
                                                                        $item->quantity;
                                                            @endphp
                                                        @elseif((str_contains($item->tax_sale, '% VAT') || str_contains($item->tax_sale, '%')) && $vat_type != 'No VAT')
                                                            @php
                                                                $rate =
                                                                    (float) preg_replace(
                                                                        '/[^\d,]/',
                                                            ...
                                                            @endphp
                                                            @if (!empty(old('currency')) && old('currency') == 'MUR')
                                                                {{ number_format($item->product_price * $rate * $item->quantity, 2, '.', ',') }}
                                                                <br><small>({{ $item->tax_sale }})</small>
                                                            @else
                                                                {{ number_format($item->product_price_converted * $rate * $item->quantity, 2, '.', ',') }}
                                                                ({{ number_format($item->product_price * $rate * $item->quantity, 2, '.', ',') }})
                                                                <br><small>({{ $item->tax_sale }})</small>
                                                            @endif
                                                            @php
                                                                $vat_amount =
                                                                    $vat_amount +
                                                                    $item->product_price * $rate * $item->quantity;
                                                                $tax_amount_converted =
                                                                    $tax_amount_converted +
                                                                    $item->product_price_converted *
                                                                        $rate *
                                                ...
                                                @endif
                                                @if ($have_sale_type == 'yes')
                                                    <td class="py-3 px-6 border">{{ $item->sales_type }}
                                                    </td>
                                                @endif
                                                <td class="py-3 px-6 border">
                                                    @if (isset($item->discount) && $item->discount > 0)
                                                        @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                            {{ number_format(($item->product_price_converted - ($item->product_price_converted * $item->discount) / 100) * $item->quantity, 2, '.', ',') }}
                                                            (Rs
                                                    ...
                                                    @else
                                                        @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                            {{ number_format($item->product_price_converted * $item->quantity, 2, '.', ',') }}
                                                            (Rs
                                                ...
                                                <td class="py-3 border">
                                                    <button data-modal-toggle="update-sales-type"
                                                        onclick="load_sales_type('{{ $item->id }}','{{ $item->sales_type }}')"
                                                        type="button" id="btn_sales_type" name="btn_sales_type"
                                            ...
                                            @php
                                                ...
                                                if (isset($item->discount) && $item->discount > 0) {
                                                    ...
                                                    $product_price_converted =
                                                        $item->product_price_converted -
                                                        ($item->product_price_converted * $item->discount) / 100;
                                                }

                                                $subtotal = $subtotal + $product_price * $item->quantity;
                                                $subtotal_converted =
                                                    $subtotal_converted + $product_price_converted * $item->quantity;
                                                ...
                                                if ($item->tax_sale == '15% VAT' && $vat_type != 'No VAT') {
                                                    if ($vat_type == 'Included in the price') {
                                                        ...
                                                        $amount_converted =
                                                            $amount_converted +
                                                            $item->quantity * $product_price_converted;
                                                    }

                                                    if ($vat_type == 'Added to the price') {
                                                        $total_amount =
                                                            $total_amount +
                                                            $item->quantity * ($product_price + $product_price * 0.15);
                                                        $amount_converted =
                                                            $amount_converted +
                                                            $item->quantity *
                                                                ($product_price + $product_price_converted);
                                                    }
                                                } elseif (
                                                    (str_contains($item->tax_sale, '% VAT') ||
                                                        str_contains($item->tax_sale, '%')) &&
                                                    $vat_type != 'No VAT'
                                                ) {
                                                    ...
                                                    if ($vat_type == 'Added to the price') {
                                                        $total_amount =
                                                            $total_amount +
                                                            $item->quantity * ($product_price + $product_price * $rate);
                                                        $amount_converted =
                                                            $amount_converted +
                                                            $item->quantity *
                                                                ($product_price + $product_price_converted);
                                                    ...
                                                    $amount_converted =
                                                        $amount_converted + $product_price_converted * $item->quantity;
                                                }
                                            @endphp
                                        @endforeach
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th colspan="2" class="py-3 px-6 border">
                                                @if ($isGlobalDiscount && $firstDiscount > 0)
                                                    <span class="px-2">{{ $totalDiscount }} %</span>
                                                @endif
                                                <button data-modal-toggle="update-total-discount" type="button"
                                                    id="update" name="update" title="Update"
                                                    class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                        height="16" fill="currentColor" class="bi bi-pencil"
                                                        viewBox="0 0 16 16">
                                        ...
                                        @if ($vat_type != 'No VAT')
                                            <tr>
                                                ...
                                                <th colspan="2" class="py-3 px-6 border">
                                                    @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                        {{ number_format($tax_amount_converted, 2, '.', ',') }} (Rs
                                            ...
                                            <tr>
                                                ...
                                                <th colspan="2" class="py-3 px-6 border">
                                                    @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                        {{ number_format($subtotal_converted, 2, '.', ',') }} (Rs
                                        ...
                                        @if ($vat_type != 'No VAT')
                                            <tr>
                                                ...
                                                <th colspan="2" class="py-3 px-6 border">
                                                    @if (!empty(old('currency')) && old('currency') != 'MUR')
                                                        {{ number_format($amount_converted, 2, '.', ',') }} (Rs
                                ...
                                @if (Session::has('data_currency') && isset(Session::get('data_currency')->conversion_rates))
                                    ...
                                    @endphp
                                @endif
                                <input data-modal-toggle="error-modal" type="hidden" name="click_btn_error_modal"
                                    id="click_btn_error_modal">
                                ...
                                <input type="hidden" name="amount_converted" id="amount_converted"
                                    value="{{ $amount_converted }}">
                                <input type="hidden" name="subtotal_converted" id="subtotal_converted"
                                    value="{{ $subtotal_converted }}">
                                <input type="hidden" name="tax_amount_converted" id="tax_amount_converted"
                                    value="{{ $tax_amount_converted }}">
                                <input type="button" id="label_click_modal" value="modal" style="display:none"
                                    data-modal-toggle="add-variation-modal" />
                                <input type="button" id="label_click_modal_store" value="modal store"
                                    style="display:none" data-modal-toggle="add-store-modal" />
                            </div>
                            <div class="grid md:grid-cols-3 md:gap-6">
                                <div class="mb-6">
                                    <div class="input-group relative flex flex-wrap items-stretch w-full">
                                        <label class="block mb-2 text-sm font-medium text-gray-900" for="currency">
                                            Sales Currency @if (isset($data_currency->conversion_rates->MUR)) : 1 {{ old('currency') }} =
                                                Rs {{ $data_currency->conversion_rates->MUR }} @endif
                                        </label>
                                        <select id="currency" name="currency"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                            <option value="MUR" @if (old('currency') == 'MUR') selected @endif>
                                                MUR</option>
                                            <option value="USD" @if (old('currency') == 'USD') selected @endif>
                                                USD</option>
                                            <option value="EUR" @if (old('currency') == 'EUR') selected @endif>
                                                EUR</option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="w-full mt-4">
                                ...
                                <div class="relative">
                                    ...
                                    <input id="delivery_date" name="delivery_date"
                                        minDate="{{ date('d/m/Y', strtotime(time())) }}"
                                        datepicker-format="dd/mm/yyyy" value="{{ old('delivery_date') }}"
                                        type="text"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 "
                                        placeholder="Expected Delivery Date" readonly="readonly">
                                </div>
                            </div>
                            <div class="w-full mt-4">
                                <label for="comment"
                            ...
                            <div class="w-full mt-4">
                                ...
                                <select id="payment_method" name="payment_method"
                                    ...
                                    @foreach ($payment_mode as $payment)
                                        @if ($payment->payment_method != 'Payment Due')
                                            <option value="{{ $payment->id }}"
                                                @if ($payment->id == old('payment_method')) selected @endif>
                                                {{ $payment->payment_method }}</option>
                                        @endif
                                    @endforeach
                                </select>
                                <small id="error_message_payment_method" class="text-red-600 ml-2 hidden">Payment Mode
                                    field is required</small>
                            </div>
                            <div class="w-full mt-4">
                                {{-- <input type="submit" name="add_sale" id="add_sale" value="Send (Draft)"
                                    class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input> --}}
                                <input type="submit" name="add_sale" id="add_sale_draft" value="Send (Pending)"
                                    class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input>
                                <input type="submit" name="add_sale" id="add_sale_paid" value="Send (Paid)"
                                    class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input>
                            </div>
                        </div>
                    </div>
            ...
            <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
                id="add-variation-modal" aria-hidden="true">
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <small id="error_message_variation" class="text-red-600 ml-2 hidden">Item field is
                                        required</small>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 gap-6">
                                <div class="col-span-12 sm:col-span-12">
                                    <div class="col-span-6 sm:col-span-3 pt-3">
                                        <label for="stock"
                                            class="text-sm font-medium text-gray-900 block mb-2">Current Stock : <span
                                                id="quantity_stock">{{ old('quantity_stock') }}</span></label>
                                    </div>
                                </div>
                            </div>
                            <div class="grid grid-cols-6 gap-6 mt-2">
                                <div class="col-span-6 sm:col-span-3">
                                    <label for="quantity_variation"
                                        class="text-sm font-medium text-gray-900 block mb-2">Quantity</label>
                                    <input type="number" name="quantity_variation" id="quantity_variation"
                                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"
                                        placeholder="Quantity" value="{{ old('quantity') }}">
                                    <small id="error_message_quantity_variation"
                                        class="text-red-600 ml-2 hidden">Invalid quantity field</small>
                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <label for="price_variation"
                                        class="text-sm font-medium text-gray-900 block mb-2">Selling Price</label>
                                    <input type="number" name="price_variation" id="price_variation"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                        value="{{ old('price_variation') }}" placeholder="Price" step=".01"
                        ...
                        <div class="p-6 border-t border-gray-200 rounded-b text-right ">
                            <input type="submit" name="add_item_variation" id="add_item_variation" value="Add Item"
                                class="text-white cursor-pointer bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center"></input>
                            <button id="cancel_button" data-modal-toggle="add-variation-modal" type="button"
                                class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
                id="add-store-modal" aria-hidden="true">
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="p-6">
                            ...
                            <select id="store_popup" name="store_popup"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                <option value="" selected>Select a Store</option>
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id }}"
                                        @if ($store->id == old('store_popup')) selected @endif>{{ $store->name }}</option>
                                @endforeach
                            </select>
                            <small id="error_message_store_popup" class="text-red-600 ml-2 hidden">Store field is
                                required</small>
                        </div>
                        <div class="p-6 border-t border-gray-200 rounded-b text-right ">
                            <input type="submit" name="select_store" id="select_store" value="Select Store"
                                class="text-white cursor-pointer bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center"></input>
                            <button id="cancel_button_store" data-modal-toggle="add-store-modal" type="button"
                                class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Cancel</button>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="align-middle inline-block min-w-full">
                                        <div class="shadow overflow-hidden sm:rounded-lg">
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                ...
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="text" id="row_item" name="row_item" placeholder="Item"
                                            ...
                                            <div class="grid md:grid-cols-2 md:gap-6">
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        onkeyup="calculatePriceDiscount()" type="number"
                                                        id="item_unit_price" name="item_unit_price"
                                                        placeholder="Unit Price" value="" step=".01">
                                                </div>
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="item_quantity">
                                            ...
                                            <div class="grid md:grid-cols-2 md:gap-6">
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        onkeyup="calculateDiscount()" type="number" id="discount"
                                            ...
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                ...
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="text" id="item_amount" name="item_amount"
                                                    placeholder="Amount" value="" readonly="readonly" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
                id="update-sales-type" aria-hidden="true">
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="flex items-start justify-between p-5 border-b rounded-t">
                            ...
                            <button type="button"
                                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                                data-modal-toggle="update-sales-type">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                            ...
                            <div class="flex flex-col mt-2">
                                <div class="overflow-x-auto rounded-lg">
                                    <div class="align-middle inline-block min-w-full">
                                        <div class="shadow overflow-hidden sm:rounded-lg">
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                <input type="hidden" name="item_id_sales_type"
                                                    id="item_id_sales_type" value="">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="item_vat">
                                                    Sales Type
                                                </label>
                                                <select id="sales_type" name="sales_type" type="text"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        ...
                        <div class="p-6 border-t border-gray-200 rounded-b">
                            <input type="submit" name="update_sales_type" id="update_sales_type"
                                value="Update Sales Type"
                                class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 cursor-pointer"></input>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                                    ...
                                    <div class="align-middle inline-block min-w-full">
                                        <div class="shadow overflow-hidden sm:rounded-lg">
                                            <div class="flex p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg"
                                                role="alert">
                                                <svg style="width:22px" xmlns="http://www.w3.org/2000/svg"
                                                ...
                                                <span class="ml-2 font-medium">Item will be added to current order,
                                                    save in Item List, and New Stock created.</span>
                                            </div>
                                            <div class="hidden modal_error_notification flex p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg"
                                                role="alert">
                                                <svg style="width:22px" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 24 24" fill="currentColor">
                                                    <defs>
                                                ...
                                                <span class="modal_error_notification_msg ml-2 font-medium">A required
                                                    field is empty.</span>
                                            </div>
                                            <div class="grid grid-cols-2">
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="add_product_name">
                                                        Name
                                                    </label>
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="add_product_name" name="add_product_name"
                                                        placeholder="Name">
                                                    <small id="error_add_product_name"
                                                        class="text-red-600 ml-2 hidden">Name Item field is
                                                        required</small>
                                                </div>
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="add_product_supplier">
                                                        Supplier
                                                    </label>
                                                        ...
                                                        @foreach ($suppliers as $supplier)
                                                            <option value="{{ $supplier->id }}">
                                                                @if (!empty($supplier->name))
                                                                    {{ $supplier->name }}@else{{ $supplier->name_person }}
                                                                    @endif @if (!empty($supplier->order_email))
                                            ...
                                            <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="add_product_description">
                                                    Description
                                                </label>
                                                <textarea name="add_product_description" rows="4"
                                                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                                            ...
                                            <div class="grid grid-cols-2">
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="add_product_selling_price">
                                                        Selling Price
                                                    </label>
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="number" step=".01" id="add_product_selling_price"
                                                        name="add_product_selling_price" placeholder="Selling Price">
                                                    <small id="error_add_product_selling_price"
                                                        class="text-red-600 ml-2 hidden">Selling Price field is
                                                        required</small>
                                                </div>
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="number" step=".01" id="add_product_buying_price"
                                                        name="add_product_buying_price" placeholder="Buying Price">
                                                    <small id="error_add_product_buying_price"
                                                        class="text-red-600 ml-2 hidden">Buying Price field is
                                                        required</small>
                                                </div>
                                            </div>
                                            <div class="grid grid-cols-2">
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="add_product_quantity">
                                                        Quantity
                                                    </label>
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="number" id="add_product_quantity"
                                                        name="add_product_quantity" placeholder="Quantity"
                                                        value="">
                                                    <small id="error_add_product_item_quantity"
                                                        class="text-red-600 ml-2 hidden">Quantity field is
                                                        required</small>
                                                </div>
                                                <div class="mb-2 relative pl-4 pr-4 z-0 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    ...
                                                    <small id="error_add_product_vat"
                                                        class="text-red-600 ml-2 hidden">VAT field is required</small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="p-2 border-t border-gray-200 rounded-b text-right">
                            <input type="submit" name="add_new_item_main" id="add_new_item_main"
                                value="Add New Item"
                                class="mr-4 text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 cursor-pointer"></input>
                        </div>
                    </div>
                </div>
            </div>
            <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
                id="add-service-item" aria-hidden="true">
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="flex items-start justify-between p-5 border-b rounded-t">
                            ...
                            <button type="button"
                                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                                data-modal-toggle="add-service-item">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                    ...
                                    <div class="align-middle inline-block min-w-full">
                                        <div class="shadow overflow-hidden sm:rounded-lg">
                                            <div class="flex p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg"
                                                ...
                                                <span class="ml-2 font-medium">Service Line Item is not saved into Item
                                                    List, nor associated with stock.</span>
                                            </div>
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="service_item">
                                                ...
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="text" id="service_item" name="service_item"
                                                    placeholder="Item" value="">
                                                <small id="error_service_item" class="text-red-600 ml-2 hidden">Item
                                                    field is required</small>
                                            </div>
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="service_item_price">
                                                    Unit Price
                                                </label>
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="number" step=".01" id="service_item_price"
                                                    name="service_item_price" placeholder="Unit Price"
                                                    value="">
                                                <small id="error_service_item_price"
                                                    class="text-red-600 ml-2 hidden">Unit Price field is
                                                    required</small>
                                            </div>
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="service_item_quantity">
                                                    Quantity
                                                </label>
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="number" id="service_item_quantity"
                                                    name="service_item_quantity" placeholder="Quantity"
                                                    value="">
                                                <small id="error_service_item_quantity"
                                                    class="text-red-600 ml-2 hidden">Quantity field is required</small>
                                            </div>
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    for="vat_service_item">
                                                    VAT
                                                </label>
                                                ...
                                                <small id="error_vat_service_item"
                                                    class="text-red-600 ml-2 hidden">VAT field is required</small>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="p-6 border-t border-gray-200 rounded-b">
                            <input type="submit" name="add_service_item" id="add_service_item"
            ...
            <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
                id="add-new-customer" aria-hidden="true">
                <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="flex items-start justify-between p-5 border-b rounded-t">
                            ...
                            <button type="button"
                                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                                data-modal-toggle="add-new-customer">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                    ...
                                    <div class="align-middle inline-block min-w-full">
                                        <div class="shadow overflow-hidden sm:rounded-lg">
                                            ...
                                            <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                ...
                                                <input
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    type="text" id="customer_company_name"
                                                    name="customer_company_name" placeholder="Company Name"
                                                    value="">
                                                <small class="text-gray-600 ml-2">If trading as individual, put
                                                    individual full name as company name</small>
                                                <small id="error_customer_company_name"
                                                    class="text-red-600 ml-2 hidden">Company Name field is
                                                    required</small>
                                            </div>
                                            <div class="grid grid-cols-2">
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_firstname"
                                                        name="customer_firstname" placeholder="First Name"
                                                        value="">
                                                </div>
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_lastname"
                                                        name="customer_lastname" placeholder="Last Name"
                                                        value="">
                                                </div>
                                            </div>
                                            <div class="grid grid-cols-3">
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_address1"
                                                        name="customer_address1" placeholder="Address 1"
                                                        value="">
                                                </div>
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                        for="customer_address2">
                                                        Address 2
                                                    </label>
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_address2"
                                                        name="customer_address2" placeholder="Address 2"
                                                        value="">
                                                </div>
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    <label class="block mb-2 text-sm font-medium text-gray-900"
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_city" name="customer_city"
                                                ...
                                                <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                                    ...
                                                    <input
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                        type="text" id="customer_country" name="customer_country"
                        ...
                        <div class="p-6 border-t border-gray-200 rounded-b">
                            <input type="submit" name="add_new_customer" id="add_new_customer"
    ...
    <form action="{{ route('newsale.update-total-discount') }}" id="main_form" method="post">
        @csrf
        <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden"
            id="update-total-discount" aria-hidden="true">
            <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
                ...
                <div class="bg-white rounded-lg shadow relative">
                    ...
                    <div class="flex items-start justify-between p-5 border-b rounded-t">
                        ...
                        <button type="button"
                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                            data-modal-toggle="update-total-discount">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd"
                                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                ...
                                <div class="align-middle inline-block min-w-full">
                                    <div class="shadow overflow-hidden sm:rounded-lg">
                                        <div class="mb-6 relative pl-4 pr-4 z-0 mb-6 w-full group">
                                            ...
                                            <input
                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                onkeyup="calculateDiscount()" type="number" id="total_discount"
                                                name="total_discount" placeholder="Total Discount (%)"
                                                value="">
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="p-6 border-t border-gray-200 rounded-b">
                        <input type="submit" name="update_item" id="update_item" value="Update"
                            class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 cursor-pointer"></input>
                    </div>
                </div>
            </div>
        ...
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <div class="flex items-start justify-between p-5 border-b rounded-t">
                    ...
                    <button type="button"
                        class="btn_error_modal text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                ...
                <div class="p-6 border-t border-gray-200 rounded-b text-right">
                    <button type="button"
                        class="btn_error_modal text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2"
                        data-modal-toggle="error-modal">Close</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        $("form").submit(function() {
            ...
            if (val === "Send (Pending)" || val === "Send (Paid)") {
                console.log(val);
                if ($("#store").val() == "") {
                ...
                if ($("#payment_method").val() == "") {
                    ...
                    show_error_modal("Payment Mode field is required");
            ...
            if (val === "Add Item") {
                if ($("#quantity_variation").val() == "" || parseFloat($("#quantity_variation")
                        .val()) <= 0) {
        ...
        $(".btn_error_modal").click(function() {
            $("#add_sale").attr("clicked", false);
            $("#add_sale_draft").attr("clicked", false);
            $("#add_sale_paid").attr("clicked", false);
        });
        ...
        $("#quantity_variation").change(function() {
            if ($("#quantity_variation").val() == "" || parseFloat($("#quantity_variation").val()) <=
                0) {
                $("#error_message_quantity_variation").show();
                $('#quantity_variation').css('border', "solid 1px red");
            } else {
        ...
        $("#payment_method").change(function() {
            const paymentMethod = $("#payment_method option:selected").text().trim();
            ...
            if (paymentMethod === "Cash") {
                $('#add_sale_draft').hide(); // Hide 'Send (Pending)'
                $('#add_sale_paid').show(); // Show 'Send (Paid)'
            } else if (paymentMethod === "Credit Sale") {
                $('#add_sale_draft').show(); // Show 'Send (Pending)'
                $('#add_sale_paid').hide(); // Hide 'Send (Paid)'
            } else {
                // For other payment methods, show both buttons
                $('#add_sale_draft').show();
                $('#add_sale_paid').show();
            }
        });
        ...
        $("#variation").change(function() {
            $("#quantity_stock").html('Not Set');
            $.each(variation_products, function(i, item) {
                if (item.id_item == $("#variation").val()) {
                    $("#price_variation").val(item.price);
                    if (item.quantity_stock === 0 || item.quantity_stock === null) {
                        $("#quantity_stock").html('Not Set');
                    } else {
                        $("#quantity_stock").html(item.quantity_stock);
                    }
                }
            });
        });
    ...
    function get_variation() {
        ...
        var url = "{{ url('product-details') }}/" + id_prod + "/" + store;
        $.ajax({
            url: url,
            type: 'GET',
            ...
            success: function(data) {
                ...
                $.each(data.variation, function(i, item) {
                    if (item.variation_value_text !== "") {
                        let variationText = item.variation_value_text.join(' | ');
                ...
                if (count > 0) {
                    ...
                    variation_products = [];
                    $("#stock_label").show();
                    $("#stock_label").html("Current Stock : " + stock + " | Selling Price : Rs " + data
                        .product.price);
                }
    ...
    function show_total_discount_modal() {
        $("#label_click_modal").click();
    }

    function load_row(id, product_id, product_name, product_price, quantity, discount, tax_sale) {
        $("#item_id").val(id);
        if (product_id == "-1") {
            $("#row_item").removeAttr("readonly");
            $("#row_item").removeAttr("disabled");
            var classes = $("#row_item").prop('class').replace('cursor-not-allowed', '');
            $("#row_item").prop('class', classes);
        } else {
            $("#row_item").attr("readonly", "readonly");
            $("#row_item").attr("disabled", true);
            var classes = $("#row_item").prop('class');
            $("#row_item").prop('class', classes + ' cursor-not-allowed');
        }
    ...
    function validatePrice(value) {
        ...
        if (parseFloat(value) <= 0) {
            errorMessage.textContent = 'Error: Selling price should be greater than 0';
