                    ...
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="mb-6 grid md:grid-cols-3 md:gap-6">
                            <div class="flex mb-2">
                                ...
                                <input name="order_reference" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/2 ml-4 p-2.5" type="text" placeholder="Order Reference" value="{{old('order_reference')}}">
                            </div>
                        ...
                        <div class="mx-auto my-2">
                                        ...
                                        <select id="customer" name="customer" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                            ...
                                            @foreach($customers as $customer)
                                                <option value="{{ $customer->id }}"
                                                        @if(Session::has('old_customer') && Session::get('old_customer') !== null)
                                                            @if(Session::get('old_customer') == $customer->id)
                                                                selected
                                                        @endif
                                                        @else
                                                            @if(empty(old('customer')))
                                                    ...
                                                    @endif
                                                >
                                                    @if(!empty($customer->company_name)) {{ $customer->company_name }} @else {{ $customer->firstname }} {{ $customer->lastname }} @endif @if(!empty($customer->email)) ({{$customer->email}})@endif</option>
                                            @endforeach
                                        </select>
                                    ...
                                    @foreach($newsale as $item)
                                        ...
                                        <tr>
                                            ...
                                            <td class="py-3 border">
                                                ...
                                                <button data-modal-toggle="update-item" onclick="load_row('{{$item->id}}','{{$item->product_id}}',`{{$item->product_name}}`,'{{$item->product_price}}','{{$item->quantity}}','{{$item->discount}}','{{$item->tax_sale}}')" type="button" id="update" name="update" title="Update" class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
                            ...
                            <div class="w-full mt-4">
                                <input type="submit" name="add_sale" id="add_sale" value="Send (Draft)" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input>
                                <input type="submit" name="add_sale" id="add_sale_draft" value="Send (Pending)" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input>
                                <input type="submit" name="add_sale" id="add_sale_paid" value="Send (Paid)" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 cursor-pointer rounded"></input>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-lg shadow relative">
                        ...
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-6 gap-6 mt-2">
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                    <label for="price_variation" class="text-sm font-medium text-gray-900 block mb-2">Selling Price</label>
                                    <input type="number" name="price_variation" id="price_variation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{old('price_variation')}}" placeholder="Price" step=".01">
...
$(document).ready(function(){
    ...
    $("#payment_method").change(function() {
        ...
        if($("#payment_method option:selected").text() == "Cash"){
            $('#add_sale').hide();
            $('#add_sale_draft').hide();
            $('#add_sale_paid').show();
        }
        else{
            if($("#payment_method option:selected").text() == "Credit Sale"){
                $('#add_sale').show();
                $('#add_sale_draft').show();
                $('#add_sale_paid').hide();
            }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(count>0){
                ...
                variation_products = [];
                $("#stock_label").show();
                $("#stock_label").html("Current Stock : " + stock + " | Selling Price : Rs " + data.product.price );
            }
