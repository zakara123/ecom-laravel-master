@php
    ...
    })->flatten()->all(); // Convert to plain array if needed

    $variationThumbnails = $product->variations->map(function ($variation) {
...
@section('customStyles')
    ...
    <style>
        ...
        .slick-slide.slick-current.slick-active {
            display: block !important; /* Force the active slide to be displayed */
        }
                        ...
                        <div class="lg:w-2/3 pswp-gallery my-gallery" style="cursor: zoom-in;" id="forcedgallery">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        ...
                                        @if (!empty($variationImages))
                                            @foreach ($variationImages as $imagev)
                                                <div class="cover-item btn_photoswipe variation-cover-image" id="cover-item{{ $i }}" data-swipe="{{ $i }}" data-variation_id="{{ $imagev->product_variation_id }}">
                                                    <img alt="{{ $product->name }}" id="item-image-{{ $i }}"
                                                         class="object-cover @if ($i == 1) active @endif object-center w-full"
                                                         src="{{ $imagev->src }}">
                                                </div>
                                                @php $i++ @endphp
                                            @endforeach
                                        ...
                                        @foreach ($images as $image)
                                            <div class="cover-item btn_photoswipe main-cover-image @if ($i == 1) active @endif" id="cover-item{{ $i }}" data-swipe="{{ $i }}">
                            ...
                            <div class="product__main_photos_mobile w-full hidden_desktop">
                                ...
                                @else
                                    ...
                                    @if (!empty($variationImages))
                                        @foreach ($variationImages as $imagev)
                                            <div class="cover-item btn_photoswipe variation-cover-image relative"
                                    ...
                                    @endif
                                    @foreach ($images as $image)
                                        <div class="cover-item relative btn_photoswipe main-cover-image @if ($i == 1) active @endif"
                                             id="cover-item{{ $i }}" data-swipe="{{ $i }}">
                                            <img alt="{{ $product->name }}" id="item-image-{{ $i }}"
                                                 class="object-cover-multiple @if ($i == 1) active @endif object-center w-full"
                        ...
                        <div class="lg:w-1/3 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            ...
                            @if (!empty($productCategory) && count($productCategory) > 0)
                                ...
                                @foreach ($productCategory as $key => $c)
                                        <a
                                            href="{{ route('category-product', $c->slug) }}">{{ $c->slug }}</a>
                            ...
                            <div class="leading-relaxed decription py-2"><b class="pb-3">Product Details</b> <br><br> <span class="py-2"> {!! $product->description !!} </span> <br></div>
...
{{--                                        </div>--}}
                                        ...
                                        <div class="flex items-center mt-2">
                                            ...
                                            <div class="flex ml-6">
                                                ...
                                                <input type="hidden" name="have_stock_api" id="have_stock_api" value="{{ $have_stock_api }}">
...
@section('customScript')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if ($('#color').length === 1 && ($("#color option:first").text() == "Choose a colour" || $("#color option:first").text() == "") || $('#color').length === 0) {
                $('#div_color').hide();
            } else {
                $('#div_color').show();
            }

            if ($('#size').length === 1 && ($("#size option:first").text() == "Choose a size" || $("#size option:first").text() == "") || $('#size').length === 0) {
                $('#div_size').hide();
            } else {
                $('#div_size').show();
            }
