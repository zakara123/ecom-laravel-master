                    ...
                    <div class="lg:w-2/3 pswp-gallery my-gallery" style="cursor: zoom-in;" id="forcedgallery">
                            ...
                            @if (count($images) >= 1 || count($variationImages) >= 1)
                                <div
                                    ...
                                    @foreach ($images as $image)
                                        <div class="thumb-image btn_photoswipe_img w-full rounded hover:opacity-90 hover:shadow-xl @if ($i == 1) active @endif"
                                    ...
                                    @if (isset($variationImages) && !empty($variationImages))
                                        @foreach ($variationImages as $imagev)
                                            <div class="thumb-image btn_photoswipe_img w-full rounded hover:opacity-90 hover:shadow-xl @if ($i == 1) active @endif variation-thumb-image"
                                                 data-src_target="item-image-{{ $i }}"
                                                 data-thumb_swipe="{{ $i }}"
                        ...
                        <div class="product__main-photos w-full hidden_mobile">
                            ...
                            @else
                                @php $i = 1 @endphp
                                @foreach ($images as $image)
                                    <div class="cover-item btn_photoswipe @if ($i == 1) active @endif"
...
<script>
    ...
    function displayVariationImages(imgVariationId){
        $('.variation-thumb-image').hide();
        $('.variation-cover-image').hide();
        $('.thumb-image').removeClass('active');
        $('.cover-item>img').removeClass('active');

        $('.variation-thumb-image').each(function() {
            var variationId = $(this).data('variation_id');
            if (variationId === imgVariationId) {
                $(this).addClass('active').show();
            }
        });

        $('.variation-cover-image').each(function() {
            var variationId = $(this).data('variation_id');
            if (variationId === imgVariationId) {
