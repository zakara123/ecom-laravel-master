<x-app-layout>
    <x-slot name="header">
        <div class="mx-4 my-4">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ url('rental-submission') }}" class="text-gray-700 hover:text-gray-900 ml-1 md:ml-2 text-sm font-medium">Rentals</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-400 ml-1 md:ml-2 text-sm font-medium" aria-current="page">Rental ID #{{$sales->id}}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <div class="grid md:grid-cols-2 md:gap-6">
                <div>
                    <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Rental ID #{{$sales->id}} from <a href="{{ route('customer-details', ['id' => $sales->customer_id]) }}" class="no-underline hover:underline">{{$sales->customer_firstname}} {{$sales->customer_lastname}}</a>
                        @if(empty($sales->order_reference))
                        <button class="ml-4" title="Update Rental Ref" data-modal-toggle="update-order-ref">
                ...
                @if(!is_null($previous))
                    <a href="{{ route('detail-rental', ['id' => $previous->id]) }}" class="text-gray-500 hover:text-gray-900 cursor-pointer p-1 hover:bg-gray-100 rounded inline-flex justify-center">
                ...
                @endif
                @if(!is_null($next))
                    <a href="{{ route('detail-rental', ['id' => $next->id]) }}" class="text-gray-500 hover:text-gray-900 cursor-pointer p-1 hover:bg-gray-100 rounded inline-flex justify-center mr-2">
            ...
            <div class="flex mt-1 ml-1 items-center">
                ...
                <button class="flex ml-4" title="Update Rental Ref" data-modal-toggle="update-order-ref">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil inline" viewBox="0 0 16 16">
                        <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z" />
                    </svg>
                </button>
    ...
    <div class="mx-auto mt-2">
        ...
        <div class="grid gap-2 mb-6 md:grid-cols-3">
            <div class="border-white rounded-md shadow px-3 py-3 bg-white">
                ...
                <h3 class="font-semibold text-xl text-center mb-3">Rental Info</h3>
                <div class="mb-2">
                ...
                <div class="mb-2">
                    <label class="block mb-2 text-sm font-medium">Amount:
                        ...
                        @else
                            Rs {{ number_format($sales->amount,2,".",",") }} / {{$sumbol_currency}} {{ number_format($sales->amount_converted,2,".",",") }}
                        @endif
                    </label>
                </div>
                </div>
            <div class="border-white rounded-md shadow px-3 py-3 bg-white">
                ...
                <div class="mb-2">
                    <label class="block mb-2 text-sm font-medium">Customer Name:  <a href="{{ route('customer-details', ['id' => $sales->customer_id]) }}" class="no-underline hover:underline"> {{$sales->customer_firstname}} {{$sales->customer_lastname}}</a></label>
                </div>
                @if(!empty($sales->customer_address))
                <div class="mb-2">
                    <label class="block mb-2 text-sm font-medium">Customer Address: {{ $sales->customer_address }}</label>
                </div>
                @endif
                @if(!empty($sales->customer_city))
                <div class="mb-2">
                    <label class="block mb-2 text-sm font-medium">Customer City: {{ $sales->customer_city }}</label>
                </div>
                @endif
                ...
                @if(!empty($sales->delivery_address))
                <div class="mb-2 mt-2">
                    <label class="block mb-2 text-sm font-medium">Delivery Address: {{ $customer->address1 }}</label>
                </div>
                @endif
                @if(!empty($sales->village_town))
                <div class="mb-2">
                    <label class="block mb-2 text-sm font-medium">Delivery Village/Town: {{ $customer->city }}</label>
                </div>
                @endif
            </div>
            ...
            <div class="border-white rounded-md shadow px-3 py-3 bg-white">
                ...
                <h3 class="font-semibold text-xl text-center mb-3">Rental Period</h3>
                
                    <div>
                        <label class="block mb-2 mt-1 text-sm font-medium">Start Date: {{ date('d/m/Y', strtotime($sales->rental_start_date)) }}</label>
                    </div>
                    <div>
                        <label class="block mb-2 mt-1 text-sm font-medium">End Date: {{ date('d/m/Y', strtotime($sales->rental_end_date)) }}</label>
                    </div>
                    <div class="text-right">
    ...
    <div class="flex flex-col bg-white rounded-md mt-6">
        <h3 class="font-semibold text-xl text-center mt-2 mb-2">Rental Items</h3>
        <div class="col-span-2 text-right mb-2">
            <!--<a  href="{{ route('rental-pending-amount-pdf',$sales->id) }}" type="button"  class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center rounded-lg text-sm px-3 py-2 text-center sm:ml-auto">
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                @if($sales->status == "Booked")
                                    <div class="flex items-center justify-center space-x-4">
                                        <button title="Update Item Sale" data-modal-toggle="update-item" onclick="updateItemSale('{{$item->product_id}}','{{ $item->tax_sale }}', '{{$item->order_price}}', '{{$item->product_variations_id}}', '{{$sales->currency}}', '{{$item->quantity}}','{{$item->frequency}}')" class="focus:outline-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil inline" viewBox="0 0 16 16">
                                                <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"></path>
                                            </svg>
                                        </button>
                                        
                                        <form action="{{ route('destroy_rental_item',$item->id) }}" method="POST"
    ...
    <div class="flex flex-col bg-white rounded-md mt-6" id="bloc_payment_sales">
            ...
            <div class="col-span-1 text-center"><h3 class="font-semibold text-sm mt-2 mb-2">Amount Due:
                ...
                <div class="col-span-1 text-center"><h3 class="font-semibold text-sm mt-2 mb-2">Amount Paid:
                    {{$sumbol_currency}} {{ number_format($amount_paied,2,".",",") }}
                ...
                <div class="col-span-2 text-right">
                    <button type="button" data-modal-toggle="add-credit-note-modal" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center rounded-lg text-sm px-3 py-2 text-center sm:ml-auto">
                        <svg class="-ml-1 mr-2 h-6 w-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path>
                        </svg>
                        Add Credit Note
                ...
                <div class="shadow overflow-hidden">
                    <table class="table-fixed min-w-full divide-y divide-gray-200">
                         ...
                         <tr>
                                ...
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">
                                    <a href="javascript:void(0)" data-modal-toggle="popup-modal-sales"
                                            onclick="view_pdf_sales('{{ route('export-rental-proforma',$sales->id) }}','rental-proforma-{{$sales->id}}.pdf?v={{ time() }}','{{$pdf_src}}')"
                                ...
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">
                                    ...
                                    <a href="{{ route('export-rental-proforma',$sales->id) }}"
                        ...
                        <tr>
                                ...
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">
                                    <a href="javascript:void(0)" data-modal-toggle="popup-modal-sales"
                                            onclick="view_pdf_sales('{{ route('export-rental',$sales->id) }}','rental-{{$sales->id}}.pdf?v={{ time() }}','{{$pdf_src}}')"
                                            class="text-gray-700 hover:text-gray-900 inline-flex items-center">
                                            invoice-{{$sales->id}}.pdf
                                    </a>
                                </td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">{{$sumbol_currency}} {{ number_format($sales->amount,2,".",",") }}</td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">
                                    {{$sumbol_currency}} {{ number_format($sales->amount,2,".",",") }}
                                </td>
                                <td class="p-4 whitespace-nowrap text-center font-medium text-gray-900">
                                    
                                    <a href="{{ route('export-rental',$sales->id) }}"
                                    class="text-gray-700 hover:text-gray-900 inline-flex items-center text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                        Download
                                    </a>
                                </td>
                        ...
                        @foreach ($sales_payments as $payment)
                            <tr>
                                ...
                                <td class="p-4 text-center font-medium text-gray-900">
                                    ...
                                    <form class="p-1" action="{{ route('rentals-payments.destroy',$payment->id) }}"
                                          method="POST"
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="add-payment-modal" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('rentals-payments.index') }}" method="POST">
                    @if(!$amount_due)
                        <div
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="add-credit-note-modal" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <div class="flex items-start justify-between p-5 border-b rounded-t">
                    ...
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-toggle="add-credit-note-modal">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                        </svg>
                    </button>
                ...
                <form action="{{ route('sales.add_credit_note') }}" method="POST">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-full">
                                <label for="creditnote_date" class="text-sm font-medium text-gray-900 block mb-2">Date</label>
                                <div class="relative">
                                    <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                                        <svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"></path>
                                    ...
                                    <input name="creditnote_date" id="creditnote_date" value="{{old('creditnote_date')}}" datepicker datepicker-autohide datepicker-format="dd/mm/yyyy" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Date Credit Note">

                                </div>
                            </div>
                            <div class="col-span-full">
                                <label for="amount_creditnote" class="text-sm font-medium text-gray-900 block mb-2">Amount</label>
                                <input type="number" name="amount" id="amount_creditnote" step=".01" min="0.01"  class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Amount"  required="">
                            </div>
                            <div class="col-span-full">
                                <label for="payment_reference" class="text-sm font-medium text-gray-900 block mb-2">Reason</label>
                                <textarea name="reason" id="reason" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Reason...">{{ old('reason') }}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="p-6 border-t border-gray-200 rounded-b">
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="update-sale-customer" aria-hidden="true">
        <div class="relative max-w-96 px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('submissions.update_customer', $sales->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid md:grid-cols-3  md:gap-6">
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    ...
                                    type="text" name="company_name" placeholder="Company Name" value="{{old('company_name', $customer->company_name)}}">
                            ...
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="brn_customer" placeholder="BRN" value="{{old('brn_customer', $customer->brn_customer)}}">
                            ...
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="vat_customer" placeholder="VAT" value="{{old('vat_customer', $customer->vat_customer)}}">
                        ...
                        <div class="grid md:grid-cols-2  md:gap-6">                            
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="firstname" placeholder="Contact First Name" value="{{old('firstname', $sales->customer_firstname)}}">
                            ...
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="lastname" placeholder="Contact Last Name" value="{{old('lastname', $sales->customer_lastname)}}">
                            </div>
                        ...
                        <div class="grid md:grid-cols-3  md:gap-6">
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="email" name="email" placeholder="Email" value="{{old('email',$sales->customer_email)}}">
                            </div>
                            <div class="mb-3 mr-2 w-full">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="phone" placeholder="Phone" value="{{old('phone',$customer->customer_phone)}}">
                            </div>
                            <div class="mb-3 mr-2 w-full">
                                <label class="block text-sm font-medium text-gray-700" for="phone">
                                    Mobile
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="mobile_no" placeholder="Mobile No" value="{{old('mobile_no',$customer->mobile_no)}}">
                            </div>
                        </div>
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="update-order-ref" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('submissions_update_order_reference', $sales->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="add-product-modal" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('submission-status-update', $sales->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-full">
                                <label for="status" class="text-sm font-medium text-gray-900 block mb-2">Retnal Status</label>
                                ...
                                @if($sales->status == 'Booked')<option value="Booked" @if($sales->status == "Booked") selected @endif>Booked</option>@endif
                                    <option value="Pending Payment" @if($sales->status == "Pending Payment") selected @endif>Pending Payment</option>
                                    <option value="Paid & Active" @if($sales->status == "Paid & Active") selected @endif>Paid & Active</option>
                                    <option value="Not Active" @if($sales->status == "Not Active") selected @endif>Not Active</option>
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="update-payment-mode-modal" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('submissions.update_rental_date', $sales->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6">
                        ...
                        <div class="w-full mt-4">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="date" name="rental_start_date" placeholder="Start Date" value="{{old('rental_start_date', $sales->rental_start_date)}}">
                            ...
                            <div class="w-full mt-4">
                                ...
                                <input
                                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400  focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="date" name="rental_end_date" placeholder="ENd Date" value="{{old('rental_end_date', $sales->rental_end_date)}}">
                            </div>
                    </div>
                    <div class="p-6 border-t border-gray-200 rounded-b">
                        <button type="submit" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update</button>
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="add-new-item" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('add-new-item-submission',$sales->id) }}" method="POST">
                    @csrf
    ...
    <div class="overflow-x-hidden overflow-y-auto fixed top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center h-modal sm:h-full hidden" id="update-item" aria-hidden="true">
        <div class="relative w-full max-w-2xl px-4 h-full md:h-auto">
            ...
            <div class="bg-white rounded-lg shadow relative">
                ...
                <form action="{{ route('update-item-submission',$sales->id) }}" method="POST">
                    @csrf
    ...
    <script type="text/javascript">
        function getProductAttributes(selectElement) {
            var selectedIndex = selectElement.selectedIndex;
            var selectedOption = selectElement.options[selectedIndex];
            var productAttributes = JSON.parse(selectedOption.getAttribute('data-attributes'));

            console.log(productAttributes);
            // Here you can access the entire object of the selected product
            // You can now update the UI or perform further operations with productAttributes
            
            $('#rental_product_name').val(productAttributes.name);
            $('#new_item_vat').val(productAttributes.vat);
            $('#add_item_unit_price').val(productAttributes.rental_price);
            //$('#add_item_currency').val(productAttributes.currency);
