            ...
            <div class="flex-col items-center mt-2 min-h-screen bg-gray-100 sm:justify-center sm:pt-0">
                ...
                <div class="w-full overflow-hidden bg-white">
                    ...
                    <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
                        <form id="upload_form_edit" method="POST" action="{{ route('variation.update', $product_variation->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <!-- Name -->
                            ...
                            <div class="col-span-full">
                                ...
                                @else
                                    <h3 class="text-md font-bold leading-none text-gray-900">{{ $product->name }} - No Variation</h3>
                                @endif
                            ...
                            <div class="grid grid-cols-3 gap-4 my-2">
                                @foreach ($attributes as $attribute)
                                    @php
                                        $selectedValue = $product_variation->attributes->firstWhere('attribute_id', $attribute->id);
...
//                                                dd($selectedValue['attribute_value_id']);
                                    ...
                                    <div class="p-1">
                                        ...
                                        <select name="{{ $attribute->id }}" required id="{{ $attribute->attribute_slug }}"
                            ...
                            <div class="grid grid-cols-6 gap-6 mt-2">
                                <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <input type="number" name="price" id="price"
                                           ...
                                           value="{{old('price',$product_variation->price)}}" min="0.10" step="0.01">

                                </div>
                                <div class="col-span-6 sm:col-span-3">
                                    <label for="price_buying" class="text-sm font-medium text-gray-900 block mb-2">Buying
                                        Price</label>
                            ...
                            @if($product_variation->id)
                                <div class="grid grid-cols-6 gap-6 mt-2">
                                    <!-- Variation Images -->
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="barcode_value" class="text-sm font-medium text-gray-900 block mb-2">Variation Image</label>
                                        <input id="filepond" class="filepond" type="file" name="item_variation_image[]" accept="image/*" multiple />
                                        @error('item_image')
                                    ...
                                    <!-- Thumbnail Image -->
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="thumbnail_image" class="text-sm font-medium text-gray-900 block mb-2">Thumbnail Image</label>
                                        <input id="filepond-thumbnail" class="filepond-thumbnail" type="file" name="thumbnail_image" accept="image/*" />
                                        ...
                                        <div class="mb-6 flex items-center w-full" id="uploadedThumbnail">
                                            @if(!empty($thumbnail))
                                                <div class="w-48 pr-4">
                                                    <div class="card text-white bg-secondary mb-3">
                                                        <div class="card-body">
                                                            <img src="{{ $thumbnail->src }}" class="card-img-top object-contain hover:object-scale-down w-48" alt="{{ $product->name }}">
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                // const variationInputElement = document.querySelector(".filepond");
                // const thumbnailInputElement = document.querySelector(".filepond-thumbnail");
                // const formElement = document.querySelector("#upload_form_edit");
                //
                // if (variationInputElement && thumbnailInputElement && formElement) {
                //     FilePond.registerPlugin(FilePondPluginImagePreview, FilePondPluginFileValidateType);
                //
                //     const pondVariation = FilePond.create(variationInputElement, {
                ...
                //     });
                //
                //     const pondThumbnail = FilePond.create(thumbnailInputElement, {
                //         acceptedFileTypes: ["image/*"],
                //         allowFileTypeValidation: true,
                //         allowFileEncode: true,
                //         instantUpload: false,
                    ...
                    formElement.addEventListener("submit", function (event) {
                        ...
                        fetch(this.action, {
                            ...
                            .then(data => {
                                ...
                                createAlertMessage(data.message, 'success', false, false);

                                // Clear FilePond files
                                // pondVariation.removeFiles();
                                // pondThumbnail.removeFiles();
            ...
            <div class="card text-white bg-secondary mb-3">
                <div class="card-body">
                    <img src="${thumbnail.src}" class="card-img-top object-contain hover:object-scale-down w-48" alt="">
                                ...
                                $("#editVariationBtn").attr("class", "text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center");
                            })
                            .catch((error) => {
                                console.error('Error:', error);
                                createAlertMessage(error.message, 'danger', false, false);
                    ...
                    console.error('Form or FilePond input not found');
                }
            });
